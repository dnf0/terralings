		...
		Chapters: []models.Chapter{
			...
			},
			{
				...
				Exercises: []models.Exercise{
					...
					{
						Name:        "state02",
						Title:       "Migrating Resources into Child Modules",
						Path:        "exercises/09_state_refactoring/state02.tf",
						ChapterName: "09_state_refactoring",
						Hints: []string{
							"Use moved { from = <resource>.<name> to = module.<mod>.<resource>.<name> }",
							"Perform zero-downtime structural refactorings",
						},
						Mode: models.ModePlan,
					},
					{
			...
			},
			{
				...
				Description: "Domain-bounded layered roots, blast radius minimization, ephemeral secrets, and comprehensions",
				Exercises: []models.Exercise{
					{
						Name:        "pattern01",
						Title:       "Decomposing Monolithic Roots into Layers",
						Path:        "exercises/11_patterns/pattern01.tf",
						ChapterName: "11_patterns",
						Hints: []string{
							"Separate network, compute, and data layers into isolated state boundaries",
							"Pass dependency outputs via data sources or remote state",
						},
						Mode: models.ModeValidate,
					},
					{
						Name:        "pattern02",
						Title:       "Minimizing Blast Radius with terraform_data",
						Path:        "exercises/11_patterns/pattern02.tf",
						ChapterName: "11_patterns",
						Hints: []string{
							"Use terraform_data instead of null_resource for zero-provider coordination",
							"Store intermediate state without external provider dependencies",
						},
						Mode: models.ModePlan,
					},
					{
						Name:        "pattern03",
						Title:       "Managing Secrets with Ephemeral Variables",
						Path:        "exercises/11_patterns/pattern03.tf",
						ChapterName: "11_patterns",
						Hints: []string{
							"Use ephemeral = true on variable and resource definitions",
							"Ephemeral values are never written to disk or state files",
						},
						Mode: models.ModePlan,
					},
					{
						Name:        "pattern04",
						Title:       "Advanced For Expressions & Comprehensions",
						Path:        "exercises/11_patterns/pattern04.tf",
						ChapterName: "11_patterns",
						Hints: []string{
							"Use [for k, v in map : v if condition] to filter and transform collections",
							"Construct complex lookup maps dynamically with { for k, v in list : k => v }",
						},
						Mode: models.ModePlan,
					},
				},
			},
			{
				...
				Description: "State encryption at rest with key_provider, early variable evaluation, and public registry",
				Exercises: []models.Exercise{
					{
						Name:        "tofu01",
						...
						Hints: []string{
							"Use encryption { key_provider \"pbkdf2\" { ... } method \"aes_gcm\" { ... } state { ... } }",
							"Encrypt state and plan files directly at rest",
						},
						Mode: models.ModeValidate,
					},
					{
						...
						ChapterName: "12_opentofu",
						Hints: []string{
							"Reference var.<name> directly within backend and provider configurations",
							"OpenTofu enables early evaluation without needing separate backend config files",
						},
						Mode: models.ModePlan,
					},
					{
						...
						ChapterName: "12_opentofu",
						Hints: []string{
							"Configure required_providers using OpenTofu registry endpoints",
			...
			},
			{
				Number:      13,
				Name:        "13_governance",
				Title:       "Linting, CI/CD & Governance",
				Description: "Strict formatting rules, policy assertions, and automated PR validation pipelines",
				Exercises: []models.Exercise{
					{
						Name:        "gov01",
						Title:       "Canonical Formatting & Layout Rules",
						Path:        "exercises/13_governance/gov01.tf",
						ChapterName: "13_governance",
						Hints: []string{
							"Enforce tofu fmt -check in CI pipelines",
							"Ensure all blocks follow standard HCL alignment",
						},
						Mode: models.ModeValidate,
					},
					{
						Name:        "gov02",
						Title:       "Policy Assertions & Environment Validation",
						Path:        "exercises/13_governance/gov02.tf",
						ChapterName: "13_governance",
						Hints: []string{
							"Use check { assert { ... } } blocks for non-blocking policy verification",
							"Check blocks report warnings during plan and apply without halting execution",
						},
						Mode: models.ModePlan,
					},
					{
						Name:        "gov03",
						Title:       "Automated PR Validation Pipelines",
						Path:        "exercises/13_governance/gov03.tf",
						ChapterName: "13_governance",
						Hints: []string{
							"Chain fmt -> validate -> plan -> test in continuous integration",
							"Catch syntax, semantic, and assertion errors before merge",
						},
						Mode: models.ModeValidate,
					},
				},
